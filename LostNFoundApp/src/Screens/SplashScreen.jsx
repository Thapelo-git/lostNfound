...
const SplashScreen = ({navigation}) => {
  ...
  const updateAvailability = (key) => { 
   db.ref('LostCard').child(key).update({Status:'Found'})
        ...
        <View style={{
           ...
           
           {
              element.StudentNumber === searchText ?(
                ...
                <Text>Add Card to Found List</Text>
                <TouchableOpacity style={styles.signinButton}
          onPress={()=>updateAvailability(element.key,)} >
                  <Text style={styles.signinButtonText}>Add Student</Text>
                  </TouchableOpacity></>
              ):(<>
              <Text>We don't have that  Student number Register Student</Text></>)
            }
